...
普通机顶盒的电视界面通常入口多、层级深、信息杂乱，老人和小孩使用遥控器时容易误触，也很难快速找到想看的频道。

HPCTV 希望把看电视这件事重新变得简单：安装并打开应用后，默认进入节目单中的第一个频道（当前为 `CCTV1`），不需要经过复杂菜单；在全屏播放界面直接按遥控器 `上键 / 下键`，即可按顺序切换频道。
